...
export async function PATCH(request: NextRequest) {
  const { phoneNumber, name, skills } = await request.json();
  if (!phoneNumber) {
  ...
  // Update name and skills
  await prisma.user.update({ where: { id: user.id }, data: { name } });
  await prisma.volunteer.update({ where: { userId: user.id }, data: { skills: JSON.stringify(skills) } });
